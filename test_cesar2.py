...
for yr in range(2010, 2025):
    ...
    if not tr.any():
        continue
    unique_classes = set(y_all[mask_tr][tr].unique())
    print(f"yr={yr}, num classes: {len(unique_classes)}, classes: {unique_classes}")
    if len(unique_classes) < 4:
        continue
    try:
        model = xgb.XGBClassifier(objective='multi:softprob')
        model.fit(X_all[mask_tr][tr], y_all[mask_tr][tr])
        print(f"yr={yr}: XGB fit OK!")
    except Exception as e:
        print(f"yr={yr}: XGB fit failed!", e)
